...
contract VestingVault is Ownable {
    ...
    struct Grant {
        ...
        uint transferred;
        uint grantedCount;       // maximun count is 2
    }
    ...
    event AddGrant (address _to, uint _amount, uint _start, uint _duration, uint _cliff, uint[] _scheduleTimes,
        uint[] _scheduleAmounts, uint _level);
    ...
    public view returns (uint, uint, uint, uint, uint[] memory, uint[] memory, uint, uint) {
        ...
        grant.scheduleValues, grant.level, grant.transferred, grant.grantedCount);
    }
    ...
    function grant(
        ...
        if (grants[_to].value == 0) {
          ...
          if (_level == 2) {
            ...
            require(grants[_to].level == _level, "The value of _level should be equal to the previous level.");
            ...
            emit AddGrant(_to, _value, _start, _duration, _cliff, _scheduleTimes, _scheduleValues, _level);
        }
